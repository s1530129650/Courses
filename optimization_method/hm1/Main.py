...
def CGM(dim ,threshold = 10**-5):
    ...
    for k in range(0, dim+1):
        ...
        if np.linalg.norm(r[k] ) < threshold:
            length = k;
...
def PlotSub(r,func,dim,threshold):
    plt.figure()
    r2norm = np.linalg.norm(r, 2, 1)
    X = np.arange(0, len(r2norm))
    length = len(X)
    ...
    ax1 = plt.subplot(3, 1, 1)
    plt.plot(X, r2norm, 'g*-')
    ax1.set_title("The error value changes as  iterating  " + "(dim = " +str(dim) +")" +"(threshold= " +str(threshold) +")")
    ax1.set_xlabel('iteration')
    ax1.set_ylabel('the value of |r|')
    #plt.xticks(range(length + 1))
    ...
    ax2 = plt.subplot(3, 1, 3)
    plt.plot(X, func, 'bo-')
    ax2.set_title("The objection function value changes as  iterating  " + "(dim = " +str(dim)+")" +"(threshold = " +str(threshold) +")")
    ax2.set_xlabel('iteration')
    ax2.set_ylabel('the value of f')
    #plt.xticks(range(length + 1))
...
if __name__ ==  '__main__':
    ...
    dim = 200
    threshold = 10**-13
    r,func,x = CGM(dim, threshold)
    #PlotTwin(r,func)
    PlotSub(r,func,dim,threshold)
    # 3.result
    #print("x: ",x)
    print("###############################################")
